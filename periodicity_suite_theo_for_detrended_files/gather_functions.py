...
def gather_aia(start_time, end_time, channel, reg,input_dist=180):
  '''
  ...
  lc_aia = LightCurve(timestamps, power, "AIA", input_wavelength=channel,
    input_region=str(reg),input_dist=input_dist)
  lc_aia.detrend(21)
  return lc_aia

def gather_data_from_file(fname,source,dir,path,input_dist=180,format=1):
  '''
...
#    Formatting in files that Cindy produces from Autoplot
...
#    Formatting in Reed's detrended files
#    There is no header line.
  ...
  if (format == 2) and ((source.upper() == 'PSP') or (source.upper() == 'STA') or (source.upper() == 'STB')):
    # In Cindy's files, this class element is expected in units of MHz
    # But not in Reed's detrends, unforunately.
    in_freq=int(regname)*(1e-6)
  else:
    in_freq=0
...
def gather_aia_fname(fname,input_dist=180):
  '''
  ...
  lc_aia = LightCurve(timestamps, power, "AIA", input_wavelength=wavelength,
              input_region=fname[-5],input_dist=input_dist)
  lc_aia.detrend(51)
  return lc_aia

def gather_psp_local_files(start_time, end_time, v_str, power_variable,input_dist=180):
  '''
    ...
    for idx, freq in enumerate(freqs):
      if power_variable == "V1V2":
        lc_list.append(LightCurve(constricted_timestamps,constricted_power[:,idx],
              'PSPV12', input_frequency=freq * 1e-6,input_dist=input_dist))
      elif power_variable == "V3V4":
        lc_list.append(LightCurve(constricted_timestamps,constricted_power[:,idx],
              'PSPV34', input_frequency=freq * 1e-6,input_dist=input_dist))
      elif power_variable == "COMBINATION":
        lc_list.append(LightCurve(constricted_timestamps,constricted_power[:,idx],
              'PSPComb', input_frequency=freq * 1e-6,input_dist=input_dist))
  # Sorting LightCurves by frequency
  lc_list.sort(key = lc_sort_func)
  return lc_list
...
def gather_nustar(start_time, end_time, fpm, region, spacing=12,input_dist=180):
  ...
  lc_nustar = LightCurve(timestamps, power, f'NuSTAR_fpm{fpm}', input_wavelength=energy,
              input_region=str(region),input_dist=input_dist)
  return lc_nustar
