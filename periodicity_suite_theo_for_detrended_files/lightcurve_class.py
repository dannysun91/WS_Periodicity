...
class LightCurve:
  ...
  '''
  def __init__(self,input_time,input_power,input_source,input_frequency=0,input_wavelength=0,
              input_regname='', input_region='',input_channel='',input_type=0,input_dist=180):
    '''
...
#    self.timestamps = input_time
    ...
    if input_source[:3].upper() == 'PSP':
      self.units = 'V^2/Hz'
      if (input_channel != ''):
        ch_parts=input_channel.split('(')
#        print('input channel   ',input_channel)
#        print('ch_parts   ',ch_parts)
        unit_str=ch_parts[1]
        self.units='('+unit_str
#        print(self.units)
    elif input_source.upper() == 'AIA':
      self.units = 'Detrended DN'
    elif input_source.upper() == 'WIND':
      self.units = 'V'
    elif input_source[:6].upper() == 'STEREO':
      self.units = 'dB above background'
    elif input_source[:6].upper() == 'NUSTAR':
...
#    self.end = datetime.fromtimestamp(self.timestamps[-1], tz=timezone.utc)
    ...
    if input_frequency != 0:
      ...
      if input_source[:6].upper() == 'NUSTAR':
        self.frequency = 1
      else:
  ...
  def get_param_msg(self):
    ...
    '''
    param_msg="Dist="+str(int(self.min_dist))
    return param_msg
  ...
  def get_param_tag(self):
    ...
    param_tag : String
      A string message giving lambda or stating n/a.
    '''
    param_tag="_dist"+str(int(self.min_dist))
    return param_tag
  ...
  def fft_periodicity(self, show_results=True, save_results=False, period_ax_range=0,
    ...
    if show_results:
      ...
      plt.show()
    if save_txt:
      txt_fname, _ = write_fnames(self, 'FFT')
      write_txt_file(txt, txt_fname)
    plt.close()
  ...
  def wavelet(self, show_results=True, save_results=False, wavelet_type='Morlet',
    ...
    #Showing results if requested
    if save_txt:
      txt_fname, _= write_fnames(self, f'{wavelet_type}Wavelet')
      write_txt_file(txt, txt_fname)
    if show_results:
      print(txt)
      plt.show()
  ...
  def summary_plot(self, bool_show=False, bool_save=False):
    '''
   ...
   #Adding in the peak finder results
    _, _, pf_period, pf_std = self.peak_finder(180, show_results=False)
    ...
    if bool_save:
      fname = write_fnames(self, 'summary')[1]
...
def split_lc(full_lc, split_dt):
  ...
  if source[:6].upper() == 'NUSTAR' or source[:3].upper() == 'AIA':
